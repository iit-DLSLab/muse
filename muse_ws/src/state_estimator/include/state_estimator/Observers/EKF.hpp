...
//TODO improve efficiency?  Cholesky factorization, back substitution
...
// public:
// 	EKF(T t0, const Eigen::Matrix<T,N,1> &xhat0, const Eigen::Matrix<T,N,N> &P0, const Eigen::Matrix<T,N,N> &Qi, const Eigen::Matrix<T,N,N> &Ri) :
// 		EKFBase<T,N,M,O,N,N,N,N>(t0,xhat0,P0,Qi,Ri) { }
// };


template <typename T, int N1, int N2, int N3, int N4, int N5, int N6, int N7> class EKFBase : public Observer<T,N1,N2,N3> {

public:
	EKFBase(T t0, const Eigen::Matrix<T,N1,1> &xhat0, const Eigen::Matrix<T,N6,N6> &P0, const Eigen::Matrix<T,N7,N7> &Qi, const Eigen::Matrix<T,N5,N5> &Ri) : Observer<T,N1,N2,N3>(t0,xhat0) {
		P=P0;
		Q=Qi;
		R=Ri;
	...
	virtual Eigen::Matrix<T,N4,N4>	calc_F(T t, const Eigen::Matrix<T,N1,1> &x, const Eigen::Matrix<T,N2,1> &u)=0;
...
//		this->P = 0.5*(this->P+this->P.transpose());
	...
	virtual std::string DebugString() const override {
		std::stringstream ss;
		ss << "===Extended Kalman Filter===" << std::endl;
		ss << "t=" << this->t_prev << std::endl;
		ss << "xhat=" << this->xhat.transpose() << std::endl;
		return ss.str();
	}

	virtual bool hasP() const override {
		return true;
	}

	void setP(const Eigen::Matrix<T,N6,N6> &P) {
		this->P = P;
	}

	void setQ(const Eigen::Matrix<T,N7,N7> &Q) {
		this->Q = Q;
	}

	void setR(const Eigen::Matrix<T,N5,N5> &R) {
		this->R = R;
	}
