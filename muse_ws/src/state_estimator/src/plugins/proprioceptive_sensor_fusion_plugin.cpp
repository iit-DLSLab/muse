...
class ProprioceptiveSensorFusionPlugin : public PluginBase  
{
	...
	~ProprioceptiveSensorFusionPlugin() {
	...
	void initialize_() override 
	{
		...
		// Default Go motor order for leg odometry computation
		motor_joint_names_ = {
		...
		proprioceptive_sensor_fusion_ = new state_estimator::KFProprioceptiveSensorFusion(t0_, xhat_estimated, P, Q, R_sf, false, false);
		...
		low_state_topic, 250,
		std::bind(&ProprioceptiveSensorFusionPlugin::callback_lowstate, this, std::placeholders::_1));
		...
		RCLCPP_INFO(node_->get_logger(), "ProprioceptiveSensorFusionPlugin initialized on topic '%s'", low_state_topic.c_str());
	}
	...
	void callback_lowstate(const unitree_go::msg::LowState::SharedPtr low_state)
	{
		...
		if (finite7(xhat_att_new)) {
			...
			RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 2000,
				"ProprioceptiveSensorFusion: attitude estimator returned NaN, keeping previous attitude");
		}
		...
		if (!w_R_b.allFinite()) {
			RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 2000,
				"ProprioceptiveSensorFusion: non-finite body rotation, using identity");
		...
		if (!finite3(u_SF) || !finite3(z_SF)) {
			RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 2000,
				"ProprioceptiveSensorFusion: non-finite filter input, skipping KF update");
			u_SF.setZero();
			z_SF.setZero();
		}

		proprioceptive_sensor_fusion_->predict(time_, u_SF);
		proprioceptive_sensor_fusion_->update(time_, z_SF);
		Eigen::Matrix<double,6,1> xhat_new = proprioceptive_sensor_fusion_->getX();
		if (finite6(xhat_new)) {
			xhat_estimated = xhat_new;
		} else {
			RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 2000,
				"ProprioceptiveSensorFusion: KF state is NaN, publishing zeros as fallback");
			xhat_estimated.setZero();
		}
