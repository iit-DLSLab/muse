...
namespace state_estimator {
    ...
    // ROS 2 time conversion functions
    inline uint64_t rclcpp_time_to_nanoseconds(rclcpp::Time t) { 
        return t.nanoseconds(); 
    }

    inline uint64_t builtin_time_to_nanoseconds(builtin_interfaces::msg::Time t) { 
        return ((uint64_t)t.sec) * 1000000000ULL + (uint64_t)t.nanosec; 
    }

    inline uint64_t chrono_time_to_nanoseconds(std::chrono::steady_clock::time_point t) {
        auto duration = t.time_since_epoch();
        return std::chrono::duration_cast<std::chrono::nanoseconds>(duration).count();
    }

    inline uint64_t rclcpp_duration_to_nanoseconds(rclcpp::Duration d) {
        ...
        // Note: ROS_WARN replaced with RCLCPP_WARN in ROS 2
        // RCLCPP_WARN(rclcpp::get_logger("lib"), "%lu:%lu:%lu:%lu:%lu:%lu:%lu:%lu",y,d,h,m,s,ms,us,ns);
		uint64_t tmp = t;
		uint64_t x = 1;
		int dec=9;
		for (dec=9;dec>-1;dec--) {
